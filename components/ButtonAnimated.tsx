...
const ButtonAnimated = ({entry}: ButtonAnimatedProps) =>  {
    ...
    const href = typeIsLink ? entry.fields.href : '';
	...
	return (
		<div className={cn('', {'text-center':  theme ==='primary' || theme ==='secondary'})}>
			<Link href={href} passHref> 
				<a
					className={cn('inline-flex py-4 px-8 m-1 text-base font-medium rounded relative z-10 overflow-hidden shadow-lg cursor-pointer', {
						...
						'bg-white text-primary-normal hover:text-white transition-colors duration-400': theme === 'secondary',
						'px-3 font-semibold hover:text-white border-l-2 border-primary-normal w-full': theme === 'tertiary'
					})}
				...
				>
					...
					<span
						className={cn(`${isHovered ? `w-[225%] h-[562.5px] `:`w-0`} btn-fill absolute -z-10 transition-{height width}`, {
							[`${!isHovered && 'h-0'} bg-primary-hover rounded-[30px] -translate-x-1/2 -translate-y-1/2 duration-500 ease-in-out `]: theme === 'primary' || theme === 'secondary',
							[`${!isHovered && 'h-full'} bg-primary-normal top-0 left-0 duration-700`]: theme === 'tertiary'
						})}
