...
const XDBanner = ({entry, sys }: IXDBanner) => {
    ...
    return (
        <div className={cn('bg-center bg-cover bg-no-repeat w-full border text-center py-12 px-8 relative z-10 before:content-[""] before:h-full before:w-full before:bg-black before:absolute before:top-0 before:left-0 before:opacity-60 before:-z-10', {
            'bg-fixed': counter
        })} style={{backgroundImage: `url(${bgImgSrc})`}} {...inspectorProps({ fieldId: 'image' })}>
            {counter && 
                <div className='flex flex-col md:flex-row justify-around items-center my-8' {...inspectorProps({ fieldId: 'counter' })}>
                    {counters}
                </div>
            }
            {description && 
                <div className='banner-description md:w-3/5 mx-auto' {...inspectorProps({ fieldId: 'description' })}>
                    <div className='mb-8'>
                        {documentToReactComponents(description, options)}
                    </div>
                    {cta && 
                        <div className='flex flex-col md:flex-row justify-center items-center' {...inspectorProps({ fieldId: 'cta' })}>
                            {buttons}
                        </div>
                    }
                </div>
