...
const XDNavItem = ({
  ...
  fixedHeader,
  isChildNavItem,
}: IXDNavigationItemProps) => {
  const navItemTransition: string = "transition duration-500";
  const navItemClasses: string = cn(
    `flex hover:text-primary-normal ${navItemTransition}`,
    {
  ...
  let childMenu: JSX.Element = entry.children?.map(
    (item: Record<string, any>, idx: number) => {
      return <XDNavItem entry={item.fields} key={idx} isChildNavItem />;
    }
  );
  ...
  return (
    <li
      className={cn("list-none ", {
        ...
        [`border border-1 hover:bg-zinc-50 ${navItemTransition} nav-child-menu-item`]:
          isChildNavItem,
      })}
    >
        ...
        {entry.slug ? (
          ...
          <div className="group relative">
            ...
            <nav
              tabIndex={0}
              className={cn(
                "bg-white h-auto w-60 rounded absolute left-0 top-16 group-hover:top-10 invisible group-hover:visible opacity-0 group-hover:opacity-100  transition-all duration-300 ease-in-out",
                {
            ...
            >
              <ul className="nav-child-menu">{childMenu}</ul>
            </nav>
          </div>
        )}
      </div>
